...
static int l4shenanigan_decap_udp(struct sk_buff *skb, unsigned int udphoff) {
  struct udphdr *udph;
  __be32 encap_daddr;
  __be16 encap_dport;
  int ret;
  ...
  udp_unfill_encap(skb, udph, encap_daddr, encap_dport);

  return 0;
}

static int l4shenanigan_decap_tcp(struct sk_buff *skb, unsigned int tcphoff) {
  struct tcphdr *tcph;
  __be32 encap_daddr;
  __be16 encap_dport;
  int ret, tcp_hdrlen;
  ...
  if (ret) {
    pr_err_ratelimited("l4shenanigan_decap_tcp: failed to ensure full tcp "
                       "header writable %d\n",
                       ret);
    return ret;
  }
...
static unsigned int l4shenanigan_decap_tg4(struct sk_buff *skb,
  ...
  switch (iph->protocol) {
  case IPPROTO_UDP:
    ret = l4shenanigan_decap_udp(skb, iph->ihl * 4);
    break;
  case IPPROTO_TCP:
    ret = l4shenanigan_decap_tcp(skb, iph->ihl * 4);
    break;
  default:
    break;
  }
...
static struct xt_target l4shenanigan_decap_tg4_regs[] __read_mostly = {{
    .name = DECAP_TARGET_NAME,
    .family = NFPROTO_IPV4,
    .target = l4shenanigan_decap_tg4,
    .targetsize = 0,
    .checkentry = l4shenanigan_decap_tg4_check,
    .destroy = l4shenanigan_decap_tg4_destroy,
    .me = THIS_MODULE,
}};

static int __init l4shenanigan_decap_tg4_init(void) {
  return xt_register_targets(l4shenanigan_decap_tg4_regs,
                             ARRAY_SIZE(l4shenanigan_decap_tg4_regs));
}

static void __exit l4shenanigan_decap_tg4_exit(void) {
  xt_unregister_targets(l4shenanigan_decap_tg4_regs,
                        ARRAY_SIZE(l4shenanigan_decap_tg4_regs));
}

module_init(l4shenanigan_decap_tg4_init);
module_exit(l4shenanigan_decap_tg4_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("L4SHENANIGAN_DECAP for iptables");
